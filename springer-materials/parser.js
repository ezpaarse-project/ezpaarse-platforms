...
module.exports = new Parser(function analyseEC(parsedUrl, ec) {
  ...
  if ((match = /^\/search$/.exec(path)) !== null) {
    ...
    if (param["data-table"]){
      if (param["data-table"].constructor === Array) {
        result.title_id = param["data-table"][0];
        var n = param["data-table"].length;
        for (var i = 1 ; i < n; i++){
          result.title_id = result.title_id + ';' + param["data-table"][i];
        }
