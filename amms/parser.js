...
module.exports = new Parser(function analyseEC(parsedUrl, ec) {
  ...
  // MathSciNet patterns (mathscinet.ams.org)
  if (hostname === 'mathscinet.ams.org') {
  ...
  // www.ams.org patterns
  else if (hostname === 'www.ams.org') {
    // Journal articles - PDF
    if ((match = /^\/journals\/([^\/]+)\/([0-9]{4})-([0-9]+)-([0-9]+)\/([A-Z0-9-]+)\/([A-Z0-9-]+)\.pdf$/.exec(path)) !== null) {
    ...
    // MathSciNet link from www.ams.org
    else if (path === '/mathscinet/' || path === '/mathscinet') {
      result.rtype    = 'SESSION';
      result.mime     = 'HTML';
    }
    // Legacy MathSciNet PDF on www.ams.org
    else if ((match = /^\/mathscinet\/pdf\/([0-9]+)\.pdf$/.exec(path)) !== null) {
    ...
    // Legacy MathSciNet search interface on www.ams.org
    // Publications search
