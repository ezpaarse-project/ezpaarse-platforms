...
module.exports = new Parser(function analyseEC(parsedUrl, ec) {
  ...
  if ((match = /^\/deliver\/fulltext\/conference\/(([0-9a-zA-Z]+)\/([0-9a-zA-Z]+).pdf)$/.exec(path)) !== null) {
    ...
    let matchDoi = /^\/content\/conferences\/([0-9a-zA-Z./]+)$/.exec(param.itemId);
    if (matchDoi) {
    ...
    let matchDoi = /^\/content\/reference\/([0-9a-zA-Z./]+)$/.exec(param.itemId);
    if (matchDoi) {
      result.doi = matchDoi[1];
    }
    ...
    let matchDoi = /^\/content\/journals\/([0-9a-zA-Z./]+)$/.exec(param.itemId);
    if (matchDoi) {
      result.doi = matchDoi[1];
    }
    ...
    let titleID = /\/([0-9a-zA-Z-?]+)/.exec(match[2]);
    if (titleID) {
