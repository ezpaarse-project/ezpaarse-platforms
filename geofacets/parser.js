...
module.exports = new Parser(function analyseEC(parsedUrl, ec) {
  ...
  if ((match = /^\/article\/(([SB])?([0-9]{7}[0-9]{5}?[0-9Xx])[0-9A-Za-z]*)\/content\/(.+)$/i.exec(path)) !== null) {
    ...
    let cgiParam = match[4];
    if (cgiParam.includes('_tbl_')) {
      result.rtype = 'TABLE';
      result.mime  = 'HTML';
    } else {
      result.rtype = 'MAP';
  ...
  } else if (/^\/figuredownload$/i.test(path)) {
  ...
  } else if (/^\/xmldownload$/i.test(path)) {
    ...
    if (format.includes('TIF')) {
      ...
      result.title_id = param.atitle;
      result.unitid   = param.mapid;
    } else if (format.includes('FULL')) {
      result.rtype    = 'IMAGE';
      result.mime     = 'JPEG';
      result.title_id = param.atitle;
      result.unitid   = param.mapid;
    } else if (format.includes('KML')) {
      result.rtype    = 'MAP';
      result.mime     = 'MISC';
      result.title_id = param.atitle;
      result.unitid   = param.mapid;
