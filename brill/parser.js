...
module.exports = new Parser(function analyseEC(parsedUrl, ec) {
  ...
  if ((match = /^\/content\/([a-z]+)\/([0-9\.]+)\/([0-9]+)$/.exec(path)) !== null) {
    ...
    result.mime     = 'PDF';
    result.issn = match[1].substr(0,4) + '-' +  match[1].substr(4,7);
    result.vol = match[3];
    ...
    result.rtype    = 'TOC';
    result.mime     = 'HTML';
    result.issn = match[3].substr(0,4) + '-' +  match[3].substr(4,7);
    result.vol = match[4];
    ...
    result.issn = match[2].substr(0,4) + '-' + match[2].substr(4,7);
    result.vol = match[3];
    result.issue = match[4];
