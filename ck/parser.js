...
module.exports = new Parser(function analyseEC(parsedUrl, ec) {
  ...
  if (/^\/ui\/service\/search$/.test(pathname)) {
  ...
  } else if ((match = /\/service\/content\/pdf\/watermarked\/(([0-9a-z-.]*)(.*?))\?/.exec(path)) !== null) {
    ...
    result.unitid   = match[1];
    result.pii      = match[3];
  } else if ((match = /\/service\/content\/ck\/(([0-9a-z-]*?)-([0-9a-z.]*?)-([0-9a-zA-Z-_]*))/.exec(path)) !== null) {
    ...
    result.mime     = 'HTML';
    result.unitid   = match[1];
  } else if (/^\/service\/browse\/drugs$/i.test(pathname)) {
