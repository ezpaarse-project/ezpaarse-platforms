...
module.exports = new Parser(function analyseEC(parsedUrl, ec) {
  ...
  if (/^\/nuevo\/nuevo_diseno\/v2\/fallo1\.asp$/i.test(path) && param.id != null) {
  ...
  } else if (/^\/nuevo\/nuevo_diseno\/v2\/doctrina1\.asp$/i.test(path) && param.id != null) {
    ...
    result.mime     = 'HTML';
    result.unitid   = param.id;
  } else if (/^\/nuevo\/nuevo_diseno\/v2\/ver-archivo-pdf\.asp$/i.test(path) && param.archivo != null) {
    ...
    result.mime     = 'PDF';
    result.unitid   = str[0];
  } else if ((match = /^\/publicador\/pdf\/([a-z0-9]+)\.docx$/i.exec(path)) != null) {
    // https://www.eldial.com/publicador/pdf/DC355C.docx
    // https://www.eldial.com/publicador/pdf/DC3556.docx
    result.rtype    = 'CODE_JURIDIQUE';
    result.mime     = 'MISC';
    result.unitid   = match[1];
  } else if (/^\/nuevo\/nuevo_diseno\/v2\/legislacion1\.asp$/i.test(path) && param.id != null && param.fecha_publicar != null) {
